(() => {
  ...
  const COUNTDOWN_DEFAULT = {configuredMs:window.ETOSCountdown.DEFAULT_DURATION_MS,status:'idle',targetTimestamp:null,pausedRemainingMs:window.ETOSCountdown.DEFAULT_DURATION_MS};
  ...
  function renderMedical(){
    ...
    let output=`<div class="medical-workstation">
      ...
      <main class="medical-content" data-medical-module="${medicalSection}"><header><div><span>ACTIVE CLINICAL MODULE</span><h2 id="medical-content-title">${titles[medicalSection]}</h2></div><strong>LOCAL RECORDS // AVAILABLE</strong></header><div class="medical-content-body">${medicalSectionContent()}</div></main>
      <aside class="medical-status-rail"><header>LAB STATUS</header><section class="${medical.genomicReviewed?'':'is-unread'}"><span>GENOMIC ANALYSIS</span><strong>COMPLETE — ${medical.genomicReviewed?'OPENED':'UNREVIEWED'}</strong></section><section><span>SPECIMEN VIALS</span><strong>${String(secure).padStart(2,'0')} SECURED</strong></section><section><span>SAMPLE H-17</span><strong>NEURAL RESPONSE PRESENT</strong></section><div class="medical-context-action">${medicalSection==='containment'?'':`<span>CONTEXT ACTION</span>${action||'<strong>NO ACTION REQUIRED</strong>'}`}</div></aside>
    </div>`;
...
if(t==='garage-door')return `<h3>GARAGE DOOR 02</h3><dl><div><dt>POSITION</dt><dd>CLOSED</dd></div><div><dt>ACTUATOR</dt><dd>STALLED</dd></div><div><dt>LOAD</dt><dd>ABOVE LIMIT</dd></div></dl><button type="button" data-garage-open>OPEN GARAGE DOOR 02</button><p class="facility-command-result" data-garage-result></p><footer>LOCAL SERVICE REQUIRED</footer>`;
  ...
  function authorizeBiosignal(){
    ...
    if(input.value===EDEM_PASSCODE){playAudio('confirm');executiveAuthorized=true;biosignalUnlocked=true;directiveUnlocked=true;facilityOverlay=null;facilityView='biosignals';renderTerminal();}
  ...
  const profiles = {
    ...
    edem: { theme:'theme-edem', title:'DR. CLAIRE EDEM // PERSONAL TERMINAL', eyebrow:'ELLISON–TANAKA OPERATING SYSTEM', status:'WS-EDM-01 // LOCAL', location:'HORIZON BASE // PERSONAL WORKSTATION', interface:'', defaultSection:'mission', render:renderEdem },
  ...
  function loadState(){ try {const saved=JSON.parse(localStorage.getItem(STORAGE_KEY)||'{}'),sanitization={...SANITIZATION_DEFAULT,...(saved.sanitization||{})},auditToken={...AUDIT_TOKEN_DEFAULT,...(saved.auditToken||{})},countdown=window.ETOSCountdown.normalize(saved.countdown||COUNTDOWN_DEFAULT);if(!['auth','timer','confirm'].includes(sanitization.resumePhase))sanitization.resumePhase='auth';if(!sanitization.keyEngaged&&['detected','turning'].includes(sanitization.phase))sanitization.phase='dormant';if(['removal-pending','removing'].includes(sanitization.phase)){sanitization.keyEngaged=false;sanitization.phase='dormant';sanitization.resumePhase='auth';sanitization.delaySeconds=0;sanitization.initiatedAt=null;sanitization.completesAt=null;sanitization.complete=false;}if(sanitization.keyEngaged&&sanitization.phase==='dormant')sanitization.phase='auth';if(sanitization.phase==='active'&&sanitization.completesAt&&Date.now()>=sanitization.completesAt)sanitization.complete=true;return {activeTerminal:'command',initialized:false,section:'overview',countdownDisplayOpen:false,...saved,sanitization,auditToken,countdown};} catch { return {activeTerminal:'command',initialized:false,section:'overview',countdownDisplayOpen:false,sanitization:{...SANITIZATION_DEFAULT},auditToken:{...AUDIT_TOKEN_DEFAULT},countdown:{...COUNTDOWN_DEFAULT}}; } }
  function saveState(){ localStorage.setItem(STORAGE_KEY,JSON.stringify(state)); }
  function showScreen(name){ [els.boot,els.terminal,els.transfer,els.countdown].forEach(el=>el.hidden=true); els[name].hidden=false; }
  ...
  let sequenceResetStatusTimer=null,countdownTimer=null,countdownExitTimer=null,countdownExitPointer=null;
  const countdownState=()=>state.countdown||(state.countdown={...COUNTDOWN_DEFAULT});
  function updateCountdownDisplay(now=Date.now()){
    const timer=countdownState(),seconds=window.ETOSCountdown.displaySeconds(timer,now),alert=window.ETOSCountdown.alertFor(seconds);
    if(timer.status==='running'&&seconds===0){timer.status='expired';timer.targetTimestamp=null;timer.pausedRemainingMs=0;saveState();}
    els.countdownValue.textContent=window.ETOSCountdown.format(seconds);els.countdownAlert.textContent=alert.text;els.countdown.dataset.countdownLevel=alert.level;
    if(els.countdownStatus){const label=timer.status==='running'?'RUNNING':timer.status==='paused'?'PAUSED':timer.status==='expired'?'EXPIRED':'READY';els.countdownStatus.textContent=`${label} // ${window.ETOSCountdown.format(seconds)}`;}
    return seconds;
  }
  function startCountdownTicker(){if(countdownTimer!==null)return;const tick=()=>{updateCountdownDisplay();syncWardenCountdown();};tick();countdownTimer=setInterval(tick,200);}
  function stopCountdownTicker(){if(countdownTimer!==null)clearInterval(countdownTimer);countdownTimer=null;}
  function syncWardenCountdown(){
    const timer=countdownState(),durationSeconds=Math.floor(timer.configuredMs/1000),hours=Math.floor(durationSeconds/3600),minutes=Math.floor((durationSeconds%3600)/60),seconds=durationSeconds%60;
    if(document.activeElement!==els.countdownHours)els.countdownHours.value=String(hours).padStart(2,'0');if(document.activeElement!==els.countdownMinutes)els.countdownMinutes.value=String(minutes).padStart(2,'0');if(document.activeElement!==els.countdownSeconds)els.countdownSeconds.value=String(seconds).padStart(2,'0');
    els.countdownSet.disabled=timer.status==='running';els.countdownStart.disabled=timer.status==='running'||timer.status==='paused'||timer.configuredMs<=0;els.countdownPause.disabled=timer.status!=='running';els.countdownResume.disabled=timer.status!=='paused'||timer.pausedRemainingMs<=0;
    updateCountdownDisplay();
  }
  function setCountdownDuration(){if(countdownState().status==='running')return;const hours=Math.max(0,Math.min(99,Number(els.countdownHours.value)||0)),minutes=Math.max(0,Math.min(59,Number(els.countdownMinutes.value)||0)),seconds=Math.max(0,Math.min(59,Number(els.countdownSeconds.value)||0)),duration=(hours*3600+minutes*60+seconds)*1000;if(duration<1000){els.countdownStatus.textContent='SET DURATION // MINIMUM 00:00:01';playAudio('reject');return;}window.ETOSCountdown.setDuration(countdownState(),duration);saveState();syncWardenCountdown();playAudio('confirm');}
  function startCountdown(){window.ETOSCountdown.start(countdownState());saveState();startCountdownTicker();syncWardenCountdown();playAudio('confirm');}
  function pauseCountdown(){window.ETOSCountdown.pause(countdownState());saveState();syncWardenCountdown();playAudio('uiBack');}
  function resumeCountdown(){window.ETOSCountdown.resume(countdownState());saveState();startCountdownTicker();syncWardenCountdown();playAudio('confirm');}
  function resetCountdown(){if(!confirm('Reset the surface extraction countdown to its configured duration?'))return;window.ETOSCountdown.reset(countdownState());saveState();syncWardenCountdown();playAudio('uiBack');}
  function adjustCountdown(deltaMs){window.ETOSCountdown.adjust(countdownState(),deltaMs);saveState();startCountdownTicker();syncWardenCountdown();playAudio('uiSelect');}
  function openCountdownDisplay(){state.countdownDisplayOpen=true;saveState();closeWarden();window.ETOSAudio?.stopAmbient();updateCountdownDisplay();showScreen('countdown');startCountdownTicker();}
  function exitCountdownDisplay(){cancelCountdownExit();state.countdownDisplayOpen=false;saveState();if(state.initialized){renderTerminal();showScreen('terminal');}else showScreen('boot');playAudio('confirm');}
  function cancelCountdownExit(){if(countdownExitTimer!==null)clearTimeout(countdownExitTimer);countdownExitTimer=null;countdownExitPointer=null;}
  function beginCountdownExit(event){if(event.pointerType==='mouse'&&event.button!==0)return;const bounds=els.countdown.getBoundingClientRect();if(event.clientX>bounds.left+bounds.width*.18||event.clientY>bounds.top+bounds.height*.22)return;cancelCountdownExit();countdownExitPointer=event.pointerId;try{els.countdown.setPointerCapture?.(event.pointerId);}catch{}countdownExitTimer=setTimeout(exitCountdownDisplay,4000);}
  function moveCountdownExit(event){if(event.pointerId!==countdownExitPointer)return;const bounds=els.countdown.getBoundingClientRect();if(event.clientX>bounds.left+bounds.width*.18||event.clientY>bounds.top+bounds.height*.22)cancelCountdownExit();}
  ...
  function syncWardenAudioDiagnostics(){
    ...
    if(versionOut)versionOut.textContent=DISPLAY_VERSION;
  ...
  function openWarden(){ cancelHold(); playAudio('confirm');const preparation=window.ETOSAudio?.prepareSanitizationAudio?.();if(preparation)void preparation.then(ok=>{syncWardenAudioDiagnostics();if(ok)window.ETOSAudio.startAmbient();});else window.ETOSAudio?.startAmbient();els.overlay.hidden=false; els.pinStep.hidden=false; els.controls.hidden=true;els.pin.value='';els.pinError.textContent='';syncWardenSanitizationDisplay();syncWardenCountdown();syncWardenAudioDiagnostics();setTimeout(()=>els.pin.focus(),50); }
  function closeWarden(){ els.overlay.hidden=true; }
  function authorize(){ if(els.pin.value===WARDEN_PIN){playAudio('confirm');els.pinStep.hidden=true;els.controls.hidden=false;els.select.value=state.activeTerminal;syncWardenSanitizationDisplay();syncWardenCountdown();syncWardenAudioDiagnostics();const prefs=loadDevPrefs(); if(els.devVisible) els.devVisible.checked=!!prefs.show;}else{playAudio('reject');els.pinError.textContent='AUTHORIZATION DENIED';els.pin.value='';} }
  ...
  function resetSession(){if(!confirm('Reset all ETOS session data and return to the boot screen?'))return;stopArgozaRecovery();window.ETOSAudio?.stopAmbient();stopSanitizationSystems();stopSanitizationFinalCountdown();stopCountdownTicker();cancelCountdownExit();window.ETOSAudio?.stopFacilityAlarm?.();window.ETOSAudio?.stopFacilityAlarmTest?.();stopMedicalHardwareWorkflows();suspendAuditTokenWorkflow();sanitizationDisplayDismissed=false;sanitizationActivationAnnouncementPending=false;localStorage.removeItem(STORAGE_KEY);state=loadState();closeWarden();renderTerminal();updateCountdownDisplay();showScreen('boot');}
  ...
  const updateClock=()=>els.clock.textContent=new Date().toLocaleTimeString('en-US',{hour12:false});updateClock();setInterval(updateClock,1000);document.querySelectorAll('[data-version-label]').forEach(label=>{label.textContent=DISPLAY_VERSION;});renderTerminal();updateCountdownDisplay();if(countdownState().status==='running'||state.countdownDisplayOpen)startCountdownTicker();showScreen(state.countdownDisplayOpen?'countdown':state.initialized?'terminal':'boot');if(!state.countdownDisplayOpen&&state.initialized&&state.activeTerminal==='argoza'&&!argozaRecoverySeen())setTimeout(startArgozaRecovery,0);console.info(`ETOS v${VERSION} loaded.`);
})();

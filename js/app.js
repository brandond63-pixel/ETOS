(() => {
  ...
  const argozaCrew = {
    away:[
      {name:'KATYA KOSMONAVT',role:'SYNTHETIC SYSTEMS SPECIALIST',primary:'Systems access / diagnostics / communications',mission:'Technical systems recovery',deployment:'HORIZON BASE AWAY TEAM'},
      ...
      {name:'DR. ALARIC FRITIGERN',role:'XENOSCIENCE RESEARCH OFFICER',primary:'Biological research assessment / sample evaluation',mission:'Scientific operations',deployment:'HORIZON BASE AWAY TEAM'},
      {name:'BUBBA SPARXXX',role:'MILITARY OPERATIONS LIAISON',primary:'Security assessment / coordination with Horizon military personnel',mission:'Operational security',deployment:'HORIZON BASE AWAY TEAM'}
    ],
    support:[
  ...
  function renderArgozaFacilityMap(mode='generic'){
    ...
    return `<div class="argoza-facility" data-argoza-facility-mode="${mode}"><section class="argoza-facility-main"><header><div><span>${mode==='mission'?'MISSION NAVIGATION REFERENCE':'FACILITY ORIENTATION // STORED REFERENCE'}</span><strong>HORIZON BASE</strong></div><button type="button" data-argoza-map-reset>RESET FACILITY VIEW</button></header><div class="argoza-facility-stage" data-argoza-facility-stage><div class="argoza-facility-canvas" data-argoza-facility-canvas style="--facility-zoom:${argozaFacilityMap.zoom};--facility-pan-x:${argozaFacilityMap.panX}px;--facility-pan-y:${argozaFacilityMap.panY}px"><div class="argoza-floorplan-wrap"><img src="assets/img/command/horizon-map/horizon-floorplan.svg" alt="Horizon Base facility floorplan">${Object.entries(argozaFacilityRooms).map(([key,room])=>`<button type="button" class="argoza-room-hotspot${key===argozaFacilityRoom?' is-selected':''}" style="--room-x:${room.x}%;--room-y:${room.y}%;--room-w:${room.w}%;--room-h:${room.h}%" data-argoza-room="${key}" aria-label="Select ${room.name}"></button>`).join('')}</div></div><p>DRAG TO PAN // PINCH OR CONTROLS TO ZOOM // SELECT AREA FOR DATA</p><div class="argoza-map-controls"><button type="button" data-argoza-map-zoom="out" aria-label="Zoom out">−</button><button type="button" data-argoza-map-zoom="in" aria-label="Zoom in">+</button></div></div></section>${sidePanel}</div>`;
  }
  ...
  async function startSanitizationActivationAudio(delaySeconds,activationTime){
    ...
    const startAlarm=reason=>{if(alarmStarted)return;alarmStarted=true;if(fallbackTimer)clearTimeout(fallbackTimer);sanitizationActivationAnnouncementPending=false;console.info(`[SanitizationAudio] requesting facility alarm${reason?` // ${reason}`:''}`);void window.ETOSAudio?.startFacilityAlarm?.();};
  ...
  function openWarden(){ cancelHold(); playAudio('confirm');const preparation=window.ETOSAudio?.prepareSanitizationAudio?.();if(preparation)void preparation.then(ok=>{syncWardenAudioDiagnostics();if(!ok)return;window.ETOSAudio.startAmbient();if(sanitizationState().phase==='active'&&!sanitizationActivationAnnouncementPending)void window.ETOSAudio.startFacilityAlarm?.();});else window.ETOSAudio?.startAmbient();els.overlay.hidden=false; els.pinStep.hidden=false; els.controls.hidden=true;els.pin.value='';els.pinError.textContent='';syncWardenSanitizationDisplay();syncWardenAudioDiagnostics();setTimeout(()=>els.pin.focus(),50); }
  ...
  function returnToBoot(){playAudio('uiBack');window.ETOSAudio?.stopAmbient();stopSanitizationSystems();stopMedicalHardwareWorkflows();suspendAuditTokenWorkflow();closeWarden();state.initialized=false;saveState();showScreen('boot');}
  ...
  els.workspace.addEventListener('click',e=>{
    if(state.initialized&&!window.ETOSAudio?.isUnlocked()){const audioUnlock=window.ETOSAudio?.unlock();if(audioUnlock)void audioUnlock.then(ok=>{if(ok){window.ETOSAudio.startAmbient();if(sanitizationState().phase==='active'&&!sanitizationActivationAnnouncementPending)void window.ETOSAudio.startFacilityAlarm?.();}});}

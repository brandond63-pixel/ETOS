...
if(t==='garage-door')return `<h3>GARAGE DOOR 02</h3><dl><div><dt>POSITION</dt><dd>CLOSED</dd></div><div><dt>ACTUATOR</dt><dd>STALLED</dd></div><div><dt>LOAD</dt><dd>ABOVE LIMIT</dd></div></dl><button type="button" data-garage-open>OPEN GARAGE DOOR 02</button><p class="facility-command-result" data-garage-result></p><footer>LOCAL SERVICE REQUIRED</footer>`;
  ...
  let sequenceResetStatusTimer=null,countdownTimer=null;
  ...
  function syncWardenCountdown(){
    ...
    els.countdownClose.hidden=!state.countdownDisplayOpen;els.countdownSet.disabled=timer.status==='running';els.countdownStart.disabled=timer.status==='running'||timer.status==='paused'||timer.configuredMs<=0;els.countdownPause.disabled=timer.status!=='running';els.countdownResume.disabled=timer.status!=='paused'||timer.pausedRemainingMs<=0;
  ...
  function exitCountdownDisplay(){cancelCountdownLogoHold();state.countdownDisplayOpen=false;saveState();closeWarden();if(state.initialized){renderTerminal();showScreen('terminal');}else showScreen('boot');playAudio('confirm');}
  function beginCountdownLogoHold(event){if(event.pointerType==='mouse'&&event.button!==0)return;if(!window.ETOSAudio?.isUnlocked())void window.ETOSAudio?.unlock();cancelCountdownLogoHold();els.countdownLogo.classList.add('is-holding');try{els.countdownLogo.setPointerCapture?.(event.pointerId);}catch{}holdTimer=setTimeout(()=>{cancelCountdownLogoHold();openWarden();},3000);}
  function cancelCountdownLogoHold(){cancelHold();els.countdownLogo.classList.remove('is-holding');}
